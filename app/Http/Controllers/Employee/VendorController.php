...
class VendorController extends Controller
{
  public function __construct()
  {
    ...
    $this->prefix= 'employee/inventory/vendor';
    $this->title= 'Vendors';
    $this->perPage= 10;
  }
  ...
  public function store(VendorStoreRequest $request)
  {
    try
    {
      ...
      if(!empty($request->name))
      {
        ...
        {
          DB::rollback();
          return redirect('vendor')->with('error','Something went wrong.');
        }
      }
    }
    catch (\Exception $e)
    {
      DB::rollback();
      $errors             = $e->getMessage();
      return redirect('vendor')->with('error',$errors);
    }
  }
  ...
  */
  public function change_vendor_status($id){
    ...
    $vendor->update();
    return redirect('/vendor')->with('success', 'Vendor status changed successfully!');
  }
}
