...
class CategoryController extends Controller
{
  public function __construct()
  {
    ...
    $this->prefix= 'pm/inventory/category';
  ...
  public function store(CategoryStoreRequest $request)
  {
    try
    {
      ...
      if(!empty($request->name))
      {
        ...
        {
          DB::commit();
          return redirect('pm/category')->with('success','Category created successfully.');
        }
        else
        {
          DB::rollback();
          return redirect('pm/category')->with('error','Something went wrong.');
        }
      }
    }
    catch (\Exception $e)
    {
      DB::rollback();
      $errors             = $e->getMessage();
      return redirect('pm/category')->with('error',$errors);
    }
  }
  ...
  public function update(CategoryStoreRequest $request, Category $category)
  {
    try
    {
      ...
      if(!empty($request->name))
      {
        ...
        {
          DB::commit();
          return redirect('pm/category')->with('success','Category updated successfully.');
        }
        else
        {
          DB::rollback();
  ...
  */
  public function change_category_status($id){
    ...
    $category->update();
    return redirect($this->url.'/category')->with('success', 'Category status changed successfully!');
  }
}
