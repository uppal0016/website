...
class DepartmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            if ($request->isMethod('post')) {
                ...
                $view = 'pm.department.search';
            } else {
                $department = Department::latest()->paginate(10);
                $view = 'pm.department.index';
            }
    ...
    public function store(Request $request)
    {
        try{
            ...
            if($department){
                return redirect('pm/department')->with('flash_message', 'Department created successfully!');
            }
        }catch(\Exception $e){
            return back()->withErrors($e);
        }
    ...
    public function edit($id)
    {
        try{
            ...
            if($department){
                return view('pm.department.edit',compact('department'));
            }else{
    ...
    public function status($id){
        try{
            ...
            $dept->update();
            if($dept){
                return redirect('/pm/department')->with('flash_message', $dept->name.' status has been changed successfully.');
            }else{
                return redirect('/pm/department')->with('flash_message', 'There is something wrong. Please try again.');
            }
        }catch(\Exception $e){
            return redirect('/pm/department')->with('flash_message', 'There is something wrong. Please try again.');
        }
