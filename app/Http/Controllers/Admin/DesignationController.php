...
class DesignationController extends Controller
{
    ...
    public function index(Request $request)
    {
        try{
            if($request->isMethod('post')){
                ...
                $view = 'admin.designations.search';
            }else{
                $designations = Designation::latest()->paginate(10);
                $view = 'admin.designations.index';
            }
             $designations->setPath('designations');
    ...
    public function store(Request $request)
    {
        try{
            ...
            if($desg){
                return redirect('/admin/designations')->with('flash_message', 'Designations has been added successfully.');
            }else{
                return redirect('/admin/designations')->with('flash_message', 'There is something wrong.');
            }
        }catch(\Exception $e){
            return redirect('/admin/designations')->with('flash_message', 'There is something wrong.');
        }
    ...
    public function update(Request $request, $id)
    {
        try{
            ...
            if($desg){
                return redirect('/admin/designations')->with('flash_message', 'Designations has been updated successfully.');
            }else{
                return redirect('/admin/designations')->with('flash_message', 'There is something wrong.');
            }
        }catch(\Exception $e){
    ...
    public function status($id){
        try{
            ...
            $desg->update();
            if($desg){
                return redirect('/admin/designations')->with('flash_message', $desg->name.' status has been changed successfully.');
            }else{
                return redirect('/admin/designations')->with('flash_message', 'There is something wrong. Please try again.');
            }
        }catch(\Exception $e){
            return redirect('/admin/designations')->with('flash_message', 'There is something wrong. Please try again.');
        }
