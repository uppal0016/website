...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
  ...
  <div class="row">
    <div class="col-lg-12">
      <h1 class="page-header">DSRs List <a href="{{ URL('add_dsr') }}" class="btn btn-primary add-user-btn add-topic-btn pull-right" style="margin-top: -7px">+ Add Dsrs </a>
      </h1> 
 <!-- <div class="panel-heading pull-right">
          <a href="{{ URL('add_dsr') }}" class="btn btn-primary add-user-btn add-topic-btn">+ Add Dsrs </a>
      </div> -->
    </div>
  </div><!--/.row-->  
          ...
          <div class="canvas-wrapper dsr-details-list">
            <div class="table-responsive">
              <table class="table ">
                  <tbody id="dsr_tbody">
                      @foreach($dsrSent as $value) 
                        ...
                        ?>
                        <tr class="dsr-point" id="dsr_{{$value['en_id']}}">
                          <td width="70%"><b>{{$project_name }}</b> <br />{{$description}}...</td>
              ...
              <span class="paginate-content">
               {{ $dsrSent->appends(\Request::except('page'))->render() }}
              </span>
            </div>
          </div>
        </div>
...
<script>
  $(document).ready(function(){
    ...
    $(document).on('click', '.dsr-point', function(){
      ...
      $.get('/drs_detail/' + getId, function(success){
        ...
        if(success != ''){
          ...
          tpl += '<div class="col-md-12" style="padding-left:0px"><p class="text-right" style="width:100%; border:1px dashed #CCC; padding:10px;"><b>Total Time Estimste:</b> ' + total + '</p></div>';
    ...
    function getExtensionType(ext){
      ...
      var imgExts = ['jpg', 'jpeg', 'png'],
          docExts = ['doc', 'docx', 'pdf', 'xls', 'xlsx', 'csv', 'txt'];
