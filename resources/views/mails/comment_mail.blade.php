            ...
            <td align="center" valign="top" style="padding: 15px;">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td align="left" style="font-size:24px;padding: 15px 0px; font-weight:bold; color:#272727; text-align: center;">
                                                <span style="margin-left:5px">DSR Reply</span></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
                        ...
                        <tr>
                            <td align="left" valign="top"><b>Hello {{$data['receiver']}},</b><br>
                               <p>This is a reply of your query on DSR report.</p>
                            </td>
                        </tr>
                    <tr>
                        <td style="color:#757575;font-family:&quot;Roboto&quot;,OpenSans,&quot;OpenSans&quot;,Arial,sans-serif;font-size:15px;font-weight:300;line-height:normal;margin:0;padding:0 25px 15px 25px;text-align:left">
                            @foreach($data['message'] as $v)
                                @if($data['userId'] == $v->user_id)
                                    <div class="col-md-12"><span><p style="font-weight: bold;">From: {{ $v->user->first_name }} {{ $v->user->last_name }} {{ '['.$v->created_at.']' }} </p></span><div><p style="margin-bottom:15px;"><b>comment: </b>{{ $v->comment }}</p></div></div>
                                @else
                                    <div class="col-md-12"><span><p style="font-weight: bold;">From: {{ $v->user->first_name }} {{ $v->user->last_name }} {{ '['.$v->created_at.']' }} </p></span><div><p style="margin-bottom:15px;"><b>comment: </b>{{ $v->comment }} </p></div></div>
                                @endif
                            @endforeach
