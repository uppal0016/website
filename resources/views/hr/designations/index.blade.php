                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="pull-right">
                                <a href="{{ URL('hr/designations/create') }}" class="btn btn-primary add-user-btn add-topic-btn">+ Add Designation </a>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table">
                                ...
                                @foreach($designations as $value)
                                    <tr>
                                        ...
                                        <td>{{$value->name}}</td>
                                        <td>
                                            @php $encryptId = Crypt::encrypt($value->id);
                                            @endphp
                                            <a href="{{ action('HR\DesignationController@edit',$encryptId) }}" title="Edit Designation">
                                            ...
                                            @endphp
                                            <a href="{{ url('/hr/designation_status',$encryptId) }}" title="<?php echo $statusText; ?>">
                                                <i class="<?php echo $icon; ?>"></i>
                                            </a> &nbsp
                                            <a href="javascript:void(0);" onclick="$(this).find('form').submit();" title="Delete Designation">
                                                <i class="fa fa-trash-o"></i>
                                                <form action="{{ url('/hr/designations',$encryptId) }}" method="post">
                                                    {{ method_field('DELETE') }}
                                                    {{ csrf_field() }}
                                                </form>
                                            </a>
